...
fn create_chars_to_priority() -> HashMap<char, u8> {
    ...
    for (i, character) in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".chars().enumerate() {
...
fn find_shared_character(strings: &Vec<String>) -> char {
    let mut sets: Vec<HashSet<char>> = strings.iter().map(|string| HashSet::from_iter(string.chars())).collect();
    let set1 = sets.pop().expect("Looks like the string vector was empty.");
    let mut shared_characters: Vec<&char> = set1.iter().filter(|c| sets.iter().all(|s| s.contains(c))).collect();
    let character = shared_characters.pop().expect("There were no shared characters...");
    ...
    loop {
        ...
        match io::stdin().read_line(&mut line) {
            Ok(num_bytes) => {
                ...
                if elves.len() == 3 {
                    let priority = priority_by_chars.get(&find_shared_character(&elves)).expect("Invalid character.");
