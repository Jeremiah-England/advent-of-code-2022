...
pub fn solve() {
    ...
    loop {
        ...
        match io::stdin().read_line(&mut line) {
            Ok(n) => {
                if n == 0 {break}
                let elf_pair: Vec<Vec<u8>> = line.trim().split(',').map(|s| s.split('-').map(|num| num.parse().unwrap()).collect()).collect();
                assert!(elf_pair.len() == 2, "There should just be two elfs on a given line.");
